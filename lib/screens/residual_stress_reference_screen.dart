...
import '../data/residual_stress_data.dart';
import '../theme/app_theme.dart';

class ResidualStressReferenceScreen extends StatefulWidget {
  const ResidualStressReferenceScreen({super.key});

  @override
  State<ResidualStressReferenceScreen> createState() => _ResidualStressReferenceScreenState();
}

class _ResidualStressReferenceScreenState extends State<ResidualStressReferenceScreen> {
  ...
  List<MapEntry<int, ReferenceSection>> get _filteredSections {
    if (_searchQuery.isEmpty) {
      return residualStressData.asMap().entries.toList();
    }

    final query = _searchQuery.toLowerCase();
    final filtered = <MapEntry<int, ReferenceSection>>[];

    for (var entry in residualStressData.asMap().entries) {
  ...
  void _expandAll() {
    setState(() {
      _expandedSections.addAll(
        List.generate(residualStressData.length, (index) => index),
      );
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: const Text('Residual Stress – Quick Reference'),
                        ...
                        return Container(
                          ...
                          child: Row(
                            children: [
                              ...
                              Expanded(
                                child: Text(
                                  'Reference overview only. Engineering evaluation required for structural decisions.',
                                  style: TextStyle(
                                    fontSize: 13,
                                    color: AppTheme.textSecondary,
                                    height: 1.4,
